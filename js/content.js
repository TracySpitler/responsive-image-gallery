...
let randomPhoto = API + 'search/photos/?page=3&query=travel&?w=400&h=400&fit=crop&' + client_id;
  ...
  .then(data => {
    ...
    for (var i = 0; i < 9; i++) {
      ...
      for (var i = 0; i < 9; i++) {
        ...
        post += '<span><img src="' + data.results[i].urls.regular + '" alt="' + data.results[i].description + '" class="image">';
